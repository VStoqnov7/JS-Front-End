function attachEvents() {
  ...
  loadBtn.addEventListener("click", (e) => loadAllBooks(e));
  loadBtn.click();
  let formBtn = document.querySelector("#form button");
  formBtn.addEventListener("click", (e) => submitForm(e));
  ...
  async function submitForm(event) {
    ...
    if (event.currentTarget.textContent === "Save") {
      alterBookEntry(title, author, _id);
    } else {
      addNewBook(title, author, _id);
    }
  }

  async function alterBookEntry(title, author, _id) {
  ...
  async function addNewBook(title, author, _id) {
    if (title === "" || author === "") {
