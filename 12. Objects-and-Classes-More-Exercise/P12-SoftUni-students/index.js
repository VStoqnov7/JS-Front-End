function solve(inputArray) {
  ...
  for (const line of inputArray) {
    if (line.includes(": ")) {
      ...
      creditsCount = parseInt(
        creditsCountwithBracket.substring(0, creditsCountwithBracket.length - 1)
      );
      let email = tokens[3];
      ...
      if (
        ...
        courses[courseName].freePlaces > 0
      ) {
  ...
  let sortedCoursesEntries = Object.entries(courses).sort(
  ...
  for (const [courseName, courseEntry] of sortedCoursesEntries) {
    ...
    let sortedStudents = courseEntry.students.sort(
