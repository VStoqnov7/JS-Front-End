function solve(inputArray) {
  ...
  for (const line of inputArray) {
    if (line.includes(" arrives")) {
      ...
      for (const leaderName of Object.keys(battleground)) {
        if (
          battleground[leaderName].armies.some((a) => a.armyName === armyName)
        ) {
          let army = battleground[leaderName].armies.filter(
            (a) => a.armyName === armyName
          )[0];
  ...
  let sortedLeaderEntries = Object.entries(battleground).sort(
    (a, b) => b[1].totalArmyCount - a[1].totalArmyCount
  );
  for (const leaderName of sortedLeaderEntries.map((l) => l[0])) {
    let armiesOutputString = "";
    let sortedArmies = battleground[leaderName].armies.sort(
      (a, b) => b.armyCount - a.armyCount
    );
    for (const army of sortedArmies) {
      armiesOutputString += `\n>>> ${army.armyName} - ${army.armyCount}`;
    }
    console.log(
      `${leaderName}: ${battleground[leaderName].totalArmyCount}` +
        armiesOutputString
    );
  }
}
