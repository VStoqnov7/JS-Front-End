  ...
  function shapeCrystal(targetThickness, currentThickness) {
    ...
    while (true) {
      ...
      function operationChange() {
        if (lastOperation !== "") {
          resultingMessage += `\r\n${
            operations[lastOperation.toLowerCase()].name
          } x${operations[lastOperation.toLowerCase()].performedCount}`;
          operations[lastOperation.toLowerCase()].performedCount = 0;
          currentThickness =
            operations.transportingAndWashing.action(currentThickness);
